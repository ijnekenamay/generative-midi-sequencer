...
*   **`DisplayController` (画面描画)**: SPI0バス（SCK: GP18, MOSI: GP19, CS: GP17, DC: GP20, RST: GP16）を介して、320x240ドットの液晶ディスプレイ ILI9341 を駆動。内蔵のピクセルフォントやカスタムアセットをメモリから読み出し、画面全幅ヘッダーやパラメータグリッド、ステッププレイヘッドを高速描画します。
...
### 5.3. パラメータアイコン表示 (Custom 8x8 Icons for Parameter Headers)
UI上部（Channel 1のさらに上）のコラムヘッダーに表示されていた従来の3文字略称テキスト（"LEN", "DEN", "SHF" 等）を廃止し、視覚的な直感性を最大化するために高精細な **カスタム8x8ピクセルアートアイコン** を全面導入しました。
*   **アイコン一覧と意匠**:
    1.  **`LEN` (長さ)**: `icon_len_8x8` ➔ 左右の開始/終了点（ループ範囲）を示すブラケット（`[  ]`）と水平軸。
    2.  **`DEN` (トリガー密度)**: `icon_den_8x8` ➔ 四方に飛び散るトリガーポイントを示すドット密度意匠。
    3.  **`SHF` (位置シフト)**: `icon_shf_8x8` ➔ 水平軸上に位置回転を示すオフセット矢印。
    4.  **`MUT` (突然変異率)**: `icon_mut_8x8` ➔ 遺伝的アルゴリズム/Turing Machineの変化を示すらせん（二重螺旋DNA）。
    5.  **`JIT` (ジッター揺らぎ)**: `icon_jit_8x8` ➔ グルーヴのマイクロタイミングの揺らぎを示すギザギザのランダム波形。
    6.  **`GAT` (ゲート音長)**: `icon_gat_8x8` ➔ 音が持続する時間の割合を示すパルス矩形波形（ゲートパルス）。
    7.  **`ROT` (ルート基音)**: `icon_rot_8x8` ➔ 音楽のアンカー・基音であることを示す美しいアンカー・クラウン（王冠）。
    8.  **`SCL` (スケール種別)**: `icon_scl_8x8` ➔ ピッチが階段状にマッピングされることを示す階段（ステアスケール）。
    9.  **`RND` (突然変異トリガー)**: `icon_rnd_8x8` ➔ 確率的シミュレーションとライブ突然変異を示す高コントラストな5点のダイス（サイコロ 🎲）。
*   **レイアウトとカラーリング**:
    - 各アイコンは 26px 幅のコラムセル内で完璧に水平方向中央揃え（`cell_x + 9`）で描画されます。
    - 各パラメータアイコンは `COLOR_GREY` でシックに表示され、最右端のライブ突然変異用 `RND` アイコンはサイバーパンクのビジュアルアクセントとして **ネオンシアン (`COLOR_CYAN`)** で常時カラーハイライトされます。

---

## 6. チャンネル別確率的パターン突然変異＆画面全体アナロググリッチフラッシュ (Stochastic Mutation & Screen Glitch System)

テクノライブパフォーマンスや偶発的なリズムジェネレーションを劇的に拡張するため、チャンネルごとに全パラメータを確率的・音楽的に「突然変異（Mutate）」させるUIコマンドおよび画面全域の超高速グリッチエフェクトを実装しています。

### 6.1. UIボタンと突然変異（RNDセル）
各トラック行のパラメータグリッドの最右端（第9列目）に、ネオンシアンで常時輝く **`RND`** セル（ボタン）を追加しました。このセルにカーソルを合わせ、`A` または `B` ボタンを押すことで、そのトラックに対して以下の**音楽的な確率的突然変異**が瞬時に施されます：

1.  **Euclidean Length（ステップ長）**: `8, 12, 16, 24, 32` ステップ（テクノで極めて実用的な拍数）の中からランダムに再抽選。
2.  **Euclidean Density（トリガー密度）**: 空パターンを避けるため、選択されたステップ長に基づき `2` から `Length/2` 個の範囲でランダムにアクティブトリガーを再生成。
3.  **Euclidean Shift（位置回転）**: `0` から `Length - 1` の範囲でステップ位置をランダムにシフト。
4.  **Turing Machine Seed（旋律/アクセントのシード値）**: Turing Machine `NoteGenerator` 内の16ビットシフトレジスタをハードウェア乱数生成器（`get_rand_32()`）を用いて完全ランダムな新しい初期シード値（シードがゼロの場合は回避）へ変更。これにより、生成される16ステップのメロディとアクセント構造が全く新しいものに置き換わります。
5.  **Clock Divide（スピード）**: `x1, x2, x3, x4, x6, x8`（クロック分周比 `24, 12, 8, 6, 4, 3`）からランダムに再選択。
6.  **Jitter & Gate（グルーヴと音長）**: `0%〜40%` の範囲でランダムなマイクロタイミングジッターを設定し、ゲート音長も `30%〜90%` でランダム化し、極めて人間味と揺らぎのあるテクノグルーヴを生成。

### 6.2. 画面全体のアナロググリッチフラッシュ (Screen-Wide Glitch Transition)
突然変異がトリガーされたその瞬間、単に値が書き換わるだけでなく、ハードウェアの突然変異を物理的な信号エラーのように表現する**画面全体のアナロググリッチトランジション**が230msにわたって発火します：

*   **動作メカニズム**:
    - グリッチ発生中（`now - screen_glitch_time < 230`）、画面全体の背景が通常の黒からマゼンタ（`COLOR_MAGENTA`）とシアン（`COLOR_CYAN`）に交互に超高速フラッシュ（フレーム時間 70ms ➔ 120ms ➔ 180ms ➔ 230ms）します。
    - 同時に、描画されるすべてのUIエレメント（ヘッダー、BPM文字、全4チャンネルの全テキスト・罫線・プレイヘッド位置）が左右（X軸方向に `-3px` ➔ `+2px` ➔ `-1px` ➔ `0px`）に激しくシェイクし、アナログテレビの垂直同期ずれのような美しいビジュアルショックを与えます。
    - フラッシュ中のすべての文字フォントとアイコン描画は即座に超高コントラストな黒（`COLOR_BLACK`）へ反転し、液晶全体の同期エラーと視認性の確保を両立します。
    - この230ms間は描画周波数が強制的に最大60Hzにロックされ、一瞬の液晶表示エラーのようなカタルシスあふれるパフォーマンス効果を提供します。
